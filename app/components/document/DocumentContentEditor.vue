...
export default {
  ...
  setup(props, { emit }) {
    ...
    function editorValue() { return textareaEl.value?.value ?? props.activeSection?.content ?? ""; }
    function updateContent(next, start, end) { if (!props.activeSection || !textareaEl.value) return; textareaEl.value.value = next; props.activeSection.content = next; nextTick(() => { textareaEl.value.focus(); textareaEl.value.setSelectionRange(start, end == null ? start : end); }); }
    function parseOrderedLine(line) {
      const match = String(line || "").match(/^(\s*)((?:\d+\.)*\d+)([.)])\s+(.*)$/);
      return match ? { indent: match[1], path: match[2], delimiter: match[3], body: match[4] } : null;
    }
    function parentPathBefore(value, lineStart, childIndentLength) {
      const previous = value.slice(0, lineStart).split("\n");
      for (let index = previous.length - 1; index >= 0; index -= 1) {
        const item = parseOrderedLine(previous[index]);
        if (item && item.indent.replace(/\t/g, "  ").length < childIndentLength) return item.path;
      }
      return "";
    }
    function indentSelection(event) {
      const textarea = textareaEl.value, value = editorValue(), start = textarea.selectionStart, end = textarea.selectionEnd;
      ...
      const ordered = lines.length === 1 && parseOrderedLine(lines[0]);
      let nextChunk;
      if (ordered) {
        const nextIndent = unindent ? ordered.indent.replace(/(?:\t| {1,2})$/, "") : `${ordered.indent}  `;
        const nextPath = unindent && ordered.path.includes(".")
          ? ordered.path.split(".").slice(0, -1).join(".")
          : unindent
            ? ordered.path
            : `${parentPathBefore(value, lineStart, nextIndent.replace(/\t/g, "  ").length) || ordered.path}.1`;
        nextChunk = `${nextIndent}${nextPath}${ordered.delimiter} ${ordered.body}`;
      } else {
        nextChunk = lines.map((line) => unindent ? line.replace(/^(?:\t| {1,2})/, "") : `  ${line}`).join("\n");
      }
      const startShift = nextChunk.length - chunk.length;
    ...
    function continueList(event) {
      ...
      const ordered = parseOrderedLine(line), task = line.match(/^(\s*)(- \[[ xX]\])\s+(.*)$/), bullet = line.match(/^(\s*)([-+*])\s+(.*)$/);
      if (!ordered && !task && !bullet) return false;
      event.preventDefault();
      const indent = ordered?.indent ?? task?.[1] ?? bullet?.[1] ?? "", body = ordered?.body ?? task?.[3] ?? bullet?.[3] ?? "";
      if (!String(body).trim()) { updateContent(value.slice(0, lineStart) + value.slice(start), lineStart); return true; }
      const nextPath = ordered ? ordered.path.split(".").map(Number).map((part, index, parts) => index === parts.length - 1 ? part + 1 : part).join(".") : "";
      const marker = ordered ? `${indent}${nextPath}${ordered.delimiter} ` : task ? `${indent}- [ ] ` : `${indent}${bullet[2]} `, insert = `\n${marker}`;
