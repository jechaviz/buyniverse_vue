<template>
  <div class="grid gap-5 p-5 2xl:grid-cols-[300px_minmax(0,1fr)]">
    <aside>
      <h3 class="text-sm font-800">Scoring</h3>
      ...
      <div class="mt-5 space-y-4">
        <label v-for="criterion in criteria" :key="criterion.key" class="block">
          <span class="mb-1.5 flex justify-between text-xs">
            <b>{{ criterion.label }}</b>
            <span class="font-bold text-brand">{{ event.weights[criterion.key] }}%</span>
          </span>
          <input
      ...
      <div class="mt-5 grid gap-2">
        ...
        >
          ...
          <p class="mt-1 text-[10px] leading-4 text-slate-500">{{ scenario.description }}</p>
        </button>
      </div>
    </aside>
    <div>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
            ...
            >
              ...
              <td class="px-4 py-3">
                ...
                <p class="mt-1 text-[10px] text-slate-400">{{ quote.compliant ? 'Compliant' : 'Policy exception' }}</p>
              </td>
              <td class="px-4 py-3 text-right font-bold">{{ formatMoney(quote.price, event.currency) }}</td>
              <td class="px-4 py-3">{{ quote.leadDays }}d</td>
              <td class="px-4 py-3">{{ quote.quality }}</td>
              <td class="px-4 py-3">
                <span :class="quote.risk > 35 ? 'text-rose-500' : 'text-emerald-500'">{{ quote.risk }}</span>
              </td>
              <td class="px-4 py-3">{{ quote.esg }}</td>
              <td class="px-4 py-3 text-right text-lg font-800 text-brand">{{ quote.score }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      <div v-if="rankedQuotes.length" class="mt-4 grid gap-3 md:grid-cols-3">
        <article
        ...
        >
          <div class="flex justify-between">
            <b class="text-xs">{{ supplierName(quote.supplierId) }}</b>
            <span class="text-xs font-800 text-brand">#{{ quote.rank }}</span>
          </div>
          <div class="mt-3 space-y-2">
            <div class="flex justify-between text-[10px]">
              <span class="text-slate-500">Price position</span><b>{{ quote.priceScore }}</b>
            </div>
            <div class="h-1.5 rounded-full bg-slate-100 dark:bg-slate-700">
              <div class="h-full rounded-full bg-brand" :style="{ width: quote.priceScore + '%' }"></div>
            </div>
            <div class="flex justify-between text-[10px]">
              <span class="text-slate-500">Delivery position</span><b>{{ quote.deliveryScore }}</b>
            </div>
            <div class="h-1.5 rounded-full bg-slate-100 dark:bg-slate-700">
              <div class="h-full rounded-full bg-sky-400" :style="{ width: quote.deliveryScore + '%' }"></div>
            </div>
          </div>
        </article>
      </div>
