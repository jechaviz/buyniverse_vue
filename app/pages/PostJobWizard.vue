<template>
  <section v-if="allowed" class="mx-auto max-w-4xl space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-xs sm:text-sm text-slate-500 dark:text-slate-400">Build a complete sourcing brief and collaboration workflow.</p>
      </div>
    ...
    <div
      ...
      role="status"
    >
      <span><i class="fa-solid fa-clock-rotate-left mr-2 text-amber-600 dark:text-amber-400"></i>A protected draft from this tab was restored.</span>
      <button class="text-xs font-bold text-rose-600 hover:underline" @click="discardRecovered">Discard draft</button>
    </div>
    ...
    <section class="panel overflow-hidden rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white/95 dark:bg-slate-900/90 shadow-card">
      <div v-if="step > 0" class="border-b border-slate-100/80 bg-slate-50/60 px-6 py-4 dark:border-slate-800/80 dark:bg-slate-950/40">
        <ol class="flex items-center justify-center gap-2 sm:gap-3" aria-label="Wizard progress">
      ...
      <div ref="wizardPanel" class="min-h-105 p-6 sm:p-8">
        ...
        <WizardStep1Assistant v-else-if="step === 1" :prompt="prompt" @update:prompt="prompt = $event" @skip="step = 2" @generate="generateBrief" />
        <WizardStep2Details v-else-if="step === 2" :project="project" :skills-text="skillsText" @update:skills-text="skillsText = $event" />
        <WizardStep3Files v-else-if="step === 3" :project="project" :format-size="formatSize" @add-file="addFile" @remove-file="removeFile" />
        <WizardStep4Team v-else-if="step === 4" :project="project" :available-team="availableTeam" :user-name="(id) => store.user(id)?.name" @add-team="addTeam" @remove-team="removeTeam" />
      ...
      <footer v-if="step > 0" class="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 px-6 py-5 dark:border-slate-700">
        <button class="btn-muted" @click="back"><i class="fa-solid fa-arrow-left mr-2"></i>Back</button>
        ...
        <div class="flex items-center gap-3">
          <span v-if="draftSavedAt" class="hidden text-[10px] font-semibold text-emerald-600 md:inline">
            <i class="fa-solid fa-shield mr-1"></i>Protected {{ draftSavedAt }}
          </span>
          <span class="required-note">Required fields</span>
          <span class="hidden text-xs text-slate-400 sm:block">Step {{ step }} of {{ totalSteps }}</span>
          <button v-if="step < totalSteps" class="btn-brand" @click="next">
            Next <i class="fa-solid fa-arrow-right ml-2"></i>
          </button>
          <button v-else class="btn-brand" @click="finish">
            <i class="fa-solid fa-rocket mr-2"></i>{{ existing ? "Save project" : "Create project" }}
          </button>
        </div>
      </footer>
    </section>
  ...
  <section v-else class="panel mx-auto max-w-xl p-10 text-center rounded-3xl border border-slate-200/80 bg-white/95 shadow-card dark:border-slate-800/80 dark:bg-slate-900/90">
    ...
    <p class="mt-2 text-sm text-slate-500">Project creation and sourcing briefs operate in the Buyer workspace.</p>
    <div class="mt-6 flex justify-center gap-3">
      <button class="btn-brand" @click="store.setMarketplaceMode('buyer')">Switch to Buy</button>
      <RouterLink to="/" class="btn-muted">Back home</RouterLink>
    </div>
  </section>
</template>
<script>
...
const WizardStep0Strategy = load("./app/pages/wizard/WizardStep0Strategy.vue?v=1");
const WizardStep1Assistant = load("./app/pages/wizard/WizardStep1Assistant.vue?v=1");
const WizardStep2Details = load("./app/pages/wizard/WizardStep2Details.vue?v=1");
const WizardStep3Files = load("./app/pages/wizard/WizardStep3Files.vue?v=1");
const WizardStep4Team = load("./app/pages/wizard/WizardStep4Team.vue?v=1");
const WizardStep5Approvers = load("./app/pages/wizard/WizardStep5Approvers.vue?v=1");

export default {
  components: { WizardStep0Strategy, WizardStep1Assistant, WizardStep2Details, WizardStep3Files, WizardStep4Team, WizardStep5Approvers },
  setup() {
    ...
    const totalSteps = 5, prompt = ref(""), skillsText = ref(""), wizardError = ref(""), draftRecovered = ref(false), draftSavedAt = ref("");
    ...
    const generateBrief = () => {
      ...
      project.value.title = prompt.value.slice(0, 60);
      project.value.description = prompt.value;
      step.value = 2;
      store.notice("Draft generated from prompt");
    };
    ...
    const next = () => {
      wizardError.value = "";
      if (step.value === 2 && (!project.value.title || !project.value.description)) {
        wizardError.value = "Please complete title and description.";
        return;
      }
    ...
    const finish = () => {
      ...
      if (existing.value) {
        ...
        store.notice("Project published!");
    ...
    return {
      store, allowed, existing, step, totalSteps, prompt, skillsText, wizardError, draftRecovered,
      draftSavedAt, project, chooseType, generateBrief, addFile, removeFile, availableTeam,
