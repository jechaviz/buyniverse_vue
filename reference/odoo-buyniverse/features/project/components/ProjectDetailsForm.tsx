...
const ProjectDetailsForm: React.FC<ProjectDetailsFormProps> = ({ project, onUpdate }) => {
  const { t } = useTranslation();
  const { users } = useAppState();
  
  const teamMembers = project.team.map(member => users.find(u => u.id === member.userId)).filter(Boolean) as User[];
