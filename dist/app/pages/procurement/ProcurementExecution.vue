<template><div class="space-y-5"><section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><article v-for="metric in metrics" :key="metric.label" class="premium-card rounded-xl border p-4"><div class="flex items-center justify-between"><span class="grid h-9 w-9 place-items-center rounded-lg" :class="metric.tone"><i class="fa-solid" :class="metric.icon"></i></span><b class="text-2xl">{{ metric.value }}</b></div><p class="mt-3 text-xs font-semibold text-slate-500">{{ metric.label }}</p><p class="mt-1 text-[10px] text-slate-400">{{ metric.note }}</p></article></section><DataTable
    ...
    /><section v-if="order" class="panel overflow-hidden rounded-2xl border border-slate-200/80 bg-white/95 shadow-card dark:border-slate-800/80 dark:bg-slate-900/90"><div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-slate-50/70 px-4 py-2 text-[11px] dark:border-slate-800/80 dark:bg-slate-950/40"><div class="flex items-center gap-1.5 text-slate-500"><RouterLink to="/procurement" class="font-semibold hover:text-brand">Procurement</RouterLink><span>/</span><RouterLink to="/procurement/execution" class="font-semibold hover:text-brand">Orders</RouterLink><span>/</span><span class="font-bold text-slate-900 dark:text-white">{{ order.id }}</span></div><div class="flex items-center gap-3"><RouterLink v-if="order.eventId" :to="`/procurement/sourcing?event=${order.eventId}`" class="font-bold text-brand hover:underline inline-flex items-center gap-1"><i class="fa-solid fa-file-signature text-[10px]"></i>RFX: {{ order.eventId }}
          </RouterLink><RouterLink v-if="order.invoiceId" :to="`/invoices/${order.invoiceId}`" class="font-bold text-violet-600 dark:text-violet-400 hover:underline inline-flex items-center gap-1"><i class="fa-solid fa-file-invoice text-[10px]"></i>Invoice: {{ order.invoiceId }}
          </RouterLink></div></div><header class="border-b border-slate-100 p-4 sm:p-5 dark:border-slate-800"><div class="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between"><div><div class="flex flex-wrap items-center gap-2"><span class="rounded-lg bg-brand-50 px-2 py-0.5 text-[10px] font-mono font-bold uppercase tracking-wider text-brand dark:bg-brand/20">{{ order.id }}</span><span class="badge rounded-lg px-2.5 py-0.5 text-[10px]" :class="statusClass(order.status)">{{ statusLabel(order.status) }}</span><span v-if="openExceptions.length" class="badge rounded-lg px-2.5 py-0.5 text-[10px] bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300"><i class="fa-solid fa-triangle-exclamation mr-1 text-[10px]"></i>{{ openExceptions.length }} issue
              </span></div><h2 class="font-head mt-1.5 text-xl sm:text-2xl font-800 tracking-tight text-slate-900 dark:text-white">{{ order.title }}</h2><div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400"><RouterLink :to="`/suppliers?supplier=${order.supplierId}`" class="flex items-center gap-1.5 font-bold text-slate-700 hover:text-brand dark:text-slate-300"><i class="fa-solid fa-building text-slate-400"></i>{{ supplier?.name || order.supplierId }}
              </RouterLink><span class="flex items-center gap-1.5"><i class="fa-solid fa-wallet text-slate-400"></i><b class="font-mono text-slate-700 dark:text-slate-300">{{ store.money(order.total, order.currency) }}</b></span><span class="flex items-center gap-1.5"><i class="fa-solid fa-warehouse text-slate-400"></i>{{ order.warehouse }}</span><span class="flex items-center gap-1.5"><i class="fa-regular fa-calendar text-slate-400"></i>{{ order.eta === 'Delivered' ? 'Delivered' : store.date(order.eta) }}</span></div></div><div class="flex flex-wrap gap-2"><button class="btn-muted text-xs py-2 px-3.5" @click="printOrder"><i class="fa-solid fa-print mr-1.5"></i>Print</button><button v-if="order.receivedPercent < 100" class="btn-brand text-xs py-2 px-3.5" @click="receiveOpen = true"><i class="fa-solid fa-box mr-1.5"></i>Receive
            </button><button v-if="order.status !== 'Closed'" class="btn-muted text-xs py-2 px-3.5" @click="closeOrder"><i class="fa-solid fa-check mr-1.5"></i>Close
            </button></div></div></header><nav class="flex gap-1 overflow-x-auto border-b border-slate-200/70 bg-slate-50/50 px-4 pt-2 dark:border-slate-700 dark:bg-slate-800/35"><button
          v-for="item in tabs"
          :key="item.key"
        ...
        ><i class="fa-solid mr-1.5" :class="item.icon"></i>{{ item.label }}
          <span v-if="item.count" class="ml-1 rounded-full bg-slate-200 px-1.5 py-0.5 text-[9px] dark:bg-slate-700">{{ item.count }}</span></button></nav><div v-if="tab === 'overview'" class="grid gap-0 xl:grid-cols-[minmax(0,1.25fr)_360px]"><div class="border-b border-slate-200/70 p-5 dark:border-slate-700 xl:border-b-0 xl:border-r"><h3 class="text-sm font-800">Milestones</h3><p class="mt-1 text-xs text-slate-500">Track progress through full receipt and reconciliation.</p><div class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><article v-for="stg in executionStages" :key="stg.label" class="rounded-xl border p-3.5" :class="stg.done ? 'border-emerald-300 bg-emerald-50/50 dark:border-emerald-500/30' : stg.current ? 'border-brand bg-brand-50/50 dark:bg-brand/10' : 'border-slate-200/70 dark:border-slate-700'"><div class="flex items-center justify-between"><span class="grid h-7 w-7 place-items-center rounded-lg text-xs" :class="stg.done ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500 dark:bg-slate-800'"><i class="fa-solid" :class="stg.icon"></i></span><i class="fa-solid" :class="stg.done ? 'fa-circle-check text-emerald-500' : 'fa-circle text-slate-300'"></i></div><b class="mt-3 block text-xs">{{ stg.label }}</b><p class="mt-1 text-[10px] text-slate-500">{{ stg.detail }}</p></article></div><div class="mt-6"><div class="flex justify-between text-xs"><b>Fulfillment</b><span class="font-bold text-brand">{{ order.receivedPercent }}%</span></div><div class="mt-2 h-2 rounded-full bg-slate-100 dark:bg-slate-700"><div class="h-full rounded-full bg-brand" :style="{ width: order.receivedPercent + '%' }"></div></div></div></div><aside class="p-5"><h3 class="text-sm font-800">Quick facts</h3><div class="mt-4 space-y-2"><div v-for="b in smartButtons" :key="b.label" class="flex justify-between items-center rounded-lg bg-slate-50 p-2.5 text-xs dark:bg-slate-800"><span class="flex items-center gap-2"><i class="fa-solid text-slate-400" :class="b.icon"></i>{{ b.label }}</span><b class="font-mono">{{ b.value }}</b></div></div></aside></div><OrderLinesTab v-else-if="tab === 'lines'" :order="order" :format-money="store.money" /><OrderMatchingTab v-else-if="tab === 'matching'" :match-documents="matchDocuments" :match-ready="matchReady" @run-match="runMatch" /><OrderExceptionsTab v-else-if="tab === 'exceptions'" :order="order" :user-name="(id) => store.user(id)?.name" @add="addException" @resolve="resolveException" /><div v-else-if="tab === 'documents'" class="grid gap-3 p-5 md:grid-cols-2 xl:grid-cols-4"><RouterLink v-if="order.invoiceId" :to="`/invoices/${order.invoiceId}`" class="rounded-xl border border-slate-200/70 p-4 hover:border-brand dark:border-slate-700"><i class="fa-solid fa-file-invoice-dollar text-xl text-brand"></i><b class="mt-3 block text-sm">Invoice</b><p class="mt-1 text-[10px] text-slate-500">{{ order.invoiceId }} · fiscal document</p></RouterLink><article class="rounded-xl border border-slate-200/70 p-4 dark:border-slate-700"><i class="fa-solid fa-file-contract text-xl text-brand"></i><b class="mt-3 block text-sm">Order</b><p class="mt-1 text-[10px] text-slate-500">{{ order.id }} · commercial terms</p></article></div><div v-else class="p-5"><div class="mx-auto max-w-4xl space-y-3"><div v-for="entry in order.audit" :key="entry.id" class="rounded-xl border border-slate-200/70 p-3 text-xs dark:border-slate-700"><div class="flex justify-between"><b class="text-xs">{{ entry.action }}</b><span class="text-[10px] text-slate-400">{{ store.date(entry.at) }}</span></div><p class="mt-1 text-slate-500">{{ entry.detail }}</p></div><p v-if="!order.audit.length" class="py-12 text-center text-slate-400">No activity recorded.</p></div></div></section><OrderReceiptModal :open="receiveOpen" :lines="receiptLines" @close="receiveOpen = false" @submit="saveReceipt" /></div></template>
<script>
