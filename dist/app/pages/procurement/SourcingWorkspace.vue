<template><div class="space-y-5"><DataTable
    ...
    /><section v-if="event" class="panel overflow-hidden rounded-2xl border border-slate-200/80 bg-white/95 shadow-card dark:border-slate-800/80 dark:bg-slate-900/90"><div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-slate-50/70 px-4 py-2 text-[11px] dark:border-slate-800/80 dark:bg-slate-950/40"><div class="flex items-center gap-1.5 text-slate-500"><RouterLink to="/procurement" class="font-semibold hover:text-brand">Procurement</RouterLink><span>/</span><RouterLink to="/procurement/sourcing" class="font-semibold hover:text-brand">Sourcing RFX</RouterLink><span>/</span><span class="font-bold text-slate-900 dark:text-white">{{ event.id }}</span></div><div class="flex items-center gap-3"><RouterLink
          ...
          ><i class="fa-solid fa-link text-[10px]"></i>PR: {{ event.requestId }}
          </RouterLink><span v-if="event.awardedSupplierId" class="inline-flex items-center gap-1 font-bold text-emerald-600 dark:text-emerald-400"><i class="fa-solid fa-trophy text-[10px]"></i>
            Awarded:
            ...
            </RouterLink></span></div></div><header class="border-b border-slate-100 p-4 sm:p-5 dark:border-slate-800"><div class="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between"><div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><span class="rounded-lg bg-brand-50 px-2.5 py-0.5 text-[10px] font-mono font-bold uppercase tracking-wider text-brand dark:bg-brand/20">{{ event.id }}</span><span class="badge rounded-lg bg-slate-100 px-2.5 py-0.5 text-[10px] font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ typeLabel(event.type) }}</span><span class="badge rounded-lg px-2.5 py-0.5 text-[10px] font-bold" :class="statusClass(event.status)"><i class="fa-solid mr-1 text-xs" :class="window.ProcurementCommon.statusMeta(event.status).icon"></i>{{ statusLabel(event.status) }}
              </span><span v-if="event.round > 1" class="badge rounded-lg bg-violet-50 px-2.5 py-0.5 text-[10px] font-bold text-violet-700 dark:bg-violet-500/10 dark:text-violet-300">Version {{ event.round }}</span></div><h2 class="font-head mt-1.5 text-xl sm:text-2xl font-800 tracking-tight text-slate-900 dark:text-white">{{ event.title }}</h2><div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400"><span class="flex items-center gap-1.5"><i class="fa-solid fa-wallet text-slate-400"></i><b class="font-mono text-slate-700 dark:text-slate-300">{{ store.money(event.budget, event.currency) }}</b></span><span class="flex items-center gap-1.5"><i class="fa-regular fa-calendar text-slate-400"></i>{{ store.date(event.deadline) }}</span><span class="flex items-center gap-1.5"><i class="fa-solid fa-users text-slate-400"></i>{{ event.invitedSupplierIds.length }} invited</span><span class="flex items-center gap-1.5"><i class="fa-solid fa-file-arrow-up text-slate-400"></i>{{ event.quotes.length }} offers</span></div></div><div class="flex flex-wrap gap-2"><button class="btn-muted text-xs py-2 px-3.5" @click="cloneEvent"><i class="fa-regular fa-copy mr-1.5 text-xs"></i>Copy
            </button><button v-if="event.status === 'Draft'" class="btn-brand text-xs py-2 px-3.5" @click="publish"><i class="fa-solid fa-tower-broadcast mr-1.5 text-xs"></i>Publish
            </button><button v-if="event.type === 'Auction'" class="btn-brand text-xs py-2 px-3.5" @click="router.push(`/procurement/auction?auction=${event.id}`)"><i class="fa-solid fa-gavel mr-1.5 text-xs"></i>Live bids
            </button><button v-if="['Published', 'Sent'].includes(event.status) && event.quotes.length" class="btn-brand text-xs py-2 px-3.5" @click="event.status = 'Comparing'; tab = 'comparison';"><i class="fa-solid fa-scale-balanced mr-1.5 text-xs"></i>Compare
        ...
        ><i class="fa-solid mr-1.5" :class="item.icon"></i>{{ item.label }}
          <span v-if="item.count" class="ml-1 rounded-full bg-slate-200 px-1.5 py-0.5 text-[9px] dark:bg-slate-700">{{ item.count }}</span></button></nav><SourcingLotsTab
        v-if="tab === 'overview'"
        :event="event"
