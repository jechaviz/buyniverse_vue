<template><div class="space-y-5"><section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><button v-for="metric in metrics" :key="metric.label" class="premium-card rounded-xl border p-4 text-left" @click="metric.action"><div class="flex items-center justify-between"><span class="grid h-9 w-9 place-items-center rounded-lg" :class="metric.tone"><i class="fa-solid" :class="metric.icon"></i></span><b class="text-2xl">{{ metric.value }}</b></div><p class="mt-3 text-xs font-semibold text-slate-500">{{ metric.label }}</p><p class="mt-1 text-[11px] text-slate-400">{{ metric.note }}</p></button></section><section class="grid gap-5 2xl:grid-cols-[minmax(0,1fr)_380px]"><DataTable
      ...
      /></section><QueueCreateModal :open="formOpen" :draft="draft" @close="closeForm" @save="saveRequest" /></div></template>
<script>
