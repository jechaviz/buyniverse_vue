      ...
      <table class="w-full min-w-190 text-left text-xs">
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
          <tr v-for="bid in bids" :key="bid.id">
            <td class="px-4 py-3">{{ clock(bid.at) }}</td>
            <td class="px-4 py-3 font-bold">{{ supplierName(bid.supplierId) }}</td>
            <td class="px-4 py-3 text-right font-800">{{ formatMoney(bid.amount, currency) }}</td>
