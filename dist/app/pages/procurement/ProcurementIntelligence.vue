<template>
  <div class="space-y-5">
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
      <article v-for="metric in metrics" :key="metric.label" class="premium-card rounded-xl border p-4">
        <div class="flex items-start justify-between">
          ...
          <span class="text-[10px] font-bold" :class="metric.delta.startsWith('+') ? 'text-emerald-500' : 'text-slate-400'">{{ metric.delta }}</span>
        </div>
        <p class="mt-4 text-xs font-semibold text-slate-500">{{ metric.label }}</p>
    ...
    <div v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <article v-for="report in reports" :key="report.id" class="premium-card rounded-xl border p-5">
        <div class="flex items-start justify-between">
          <span class="grid h-10 w-10 place-items-center rounded-xl bg-brand-50 text-brand"><i class="fa-solid" :class="report.icon"></i></span>
          <span class="badge bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">Ready</span>
        </div>
        <h2 class="mt-4 text-base font-800">{{ report.title }}</h2>
        <p class="mt-2 text-xs leading-5 text-slate-500">{{ report.description }}</p>
        <div class="mt-5 flex items-center justify-between">
          <span class="text-[10px] text-slate-400">Updated today</span>
          <button class="text-xs font-bold text-brand" @click="exportReport(report.id)">
