  ...
  <div class="panel overflow-hidden">
    <header class="flex flex-col gap-3 border-b border-slate-200/70 p-5 dark:border-slate-700 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="mt-1 text-xs text-slate-500">Requests, decisions, offers, receipts, issues and rule changes.</p>
      </div>
      <div class="flex gap-2">
        ...
        <button class="btn-muted" @click="$emit('export')"><i class="fa-solid fa-download"></i>Export</button>
      </div>
    </header>
      ...
      <table class="w-full min-w-210 text-left text-xs">
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
          <tr v-for="entry in filteredAudit" :key="entry.id">
            ...
            <td class="px-4 py-3 font-mono font-bold">{{ entry.entityId || "SYSTEM" }}</td>
            <td class="px-4 py-3 font-bold">{{ entry.action }}</td>
            <td class="px-4 py-3 text-slate-500">{{ userName(entry.actorId) || entry.actorId || "System" }}</td>
            <td class="px-4 py-3 text-slate-500 max-w-xs truncate">{{ entry.detail }}</td>
            <td class="px-4 py-3">
