  ...
  <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
    <article class="panel overflow-hidden">
      <header class="flex items-center justify-between border-b border-slate-200/70 p-5 dark:border-slate-700">
        <div>
          ...
          <p class="mt-1 text-xs text-slate-500">Simple triggers and clear steps.</p>
        </div>
        ...
        <article v-for="workflow in workflows" :key="workflow.id" class="p-5">
          <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p class="ml-10 mt-1 text-[10px] text-slate-500">When {{ workflow.trigger }}</p>
            </div>
            <div class="flex gap-4 text-center">
              <div><b class="block text-sm">{{ workflow.runs }}</b><small class="text-[9px] text-slate-400">Runs</small></div>
              <div><b class="block text-sm">{{ workflow.successRate }}%</b><small class="text-[9px] text-slate-400">Success</small></div>
    ...
    <aside class="space-y-5">
      <article class="panel p-5">
        <span class="text-[10px] font-800 uppercase tracking-wide text-brand">Automation health</span>
        <div class="mt-4 grid grid-cols-2 gap-2">
          <div class="rounded-lg bg-slate-50 p-3 text-center dark:bg-slate-800"><b class="block text-2xl">36</b><small class="text-[9px] text-slate-400">Total runs</small></div>
          <div class="rounded-lg bg-slate-50 p-3 text-center dark:bg-slate-800"><b class="block text-2xl text-emerald-500">94%</b><small class="text-[9px] text-slate-400">Success</small></div>
        </div>
        <p class="mt-4 text-xs leading-5 text-slate-500">Automation routes routine work; people approve important decisions.</p>
      </article>
      <article class="rounded-xl bg-slate-950 p-5 text-white">
        <span class="text-[10px] font-800 uppercase tracking-wide text-brand-100">Safety</span>
        <h3 class="mt-2 text-lg font-800">Always actionable</h3>
        <p class="mt-2 text-xs leading-5 text-slate-400">Every state has an owner and a next action. Important decisions always require a person.</p>
      </article>
    </aside>
  </div>
</template>
