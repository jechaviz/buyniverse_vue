<template>
  <div class="p-5">
    <div class="flex items-center justify-between">
      <div>
        <h3 class="text-sm font-800">Issues</h3>
        <p class="mt-1 text-xs text-slate-500">Each issue has an owner, next action and history.</p>
      </div>
      <button class="btn-muted" @click="$emit('add')">
        <i class="fa-solid fa-plus mr-1"></i>Add issue
      </button>
    </div>
    <div class="mt-4 space-y-3">
      ...
      >
        ...
        >
          <i class="fa-solid" :class="exception.status === 'Resolved' ? 'fa-check' : 'fa-triangle-exclamation'"></i>
        </span>
        <div>
          <div class="flex flex-wrap items-center gap-2">
            <b class="text-sm">{{ exception.type }}</b>
            <span class="badge" :class="exception.status === 'Resolved' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'">
              {{ exception.status }}
            </span>
            <span class="text-[10px] font-bold text-slate-400">{{ exception.severity }}</span>
          </div>
          <p class="mt-2 text-xs leading-5 text-slate-500">{{ exception.detail }}</p>
          <p class="mt-2 text-[10px] text-slate-400">Owner: {{ userName(exception.ownerId) || "Unassigned" }}</p>
        </div>
        <button
      ...
      >
        ...
        <p class="mt-2 text-sm font-bold">No open issues</p>
      </div>
    </div>
  </div>
</template>
