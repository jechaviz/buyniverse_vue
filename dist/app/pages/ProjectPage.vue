<template>
  <section v-if="job && allowed" class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        <h1 class="premium-title text-3xl font-800">{{ job.title }}</h1>
    ...
    <div v-if="contest" class="panel border-brand-200 p-5">
      <p class="text-sm font-bold text-brand">Design contest</p>
    ...
    <section v-if="isPreHiring" class="panel p-5">
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p class="premium-kicker text-xs font-bold uppercase text-brand">Project launchpad</p>
          <h2 class="mt-2 text-xl font-800">Source, compare and award providers</h2>
        </div>
        <button v-if="job.clientId === store.currentUser.value.id && job.status === 'DRAFT'" class="btn-brand" @click="publishProject">
          <i class="fa-solid fa-rocket mr-2"></i>Publish project
        </button>
      </div>
      <ol class="mt-6 grid gap-4 md:grid-cols-3">
        <li v-for="step in launchSteps" :key="step.label" class="flex gap-3 rounded-xl bg-slate-50 p-4 dark:bg-slate-800/65">
          <span class="grid h-8 w-8 flex-none place-items-center rounded-full text-xs font-bold" :class="step.done ? 'bg-green-500 text-white' : step.current ? 'bg-brand text-white' : 'bg-slate-200 text-slate-500 dark:bg-slate-700'">
            {{ step.done ? "✓" : step.number }}
          </span>
          <div><b class="text-sm">{{ step.label }}</b><p class="mt-1 text-xs text-slate-500">{{ step.copy }}</p></div>
        </li>
      </ol>
    </section>

    <nav class="flex gap-5 overflow-x-auto border-b border-slate-200 dark:border-slate-700" aria-label="Project sections">
      <button
        v-for="tab in tabs"
        :key="tab.key"
    ...
    <!-- Files Tab -->
    <section v-else-if="activeTab === 'files'" class="panel overflow-hidden">
      <div class="border-b border-slate-100 p-5 dark:border-slate-700"><h2 class="font-800">Files</h2></div>
      <div v-if="files.length" class="divide-y divide-slate-100 dark:divide-slate-700">
        <article v-for="file in files" :key="file.id" class="flex flex-wrap items-center justify-between gap-4 p-5">
          <span class="font-mono text-sm font-bold">{{ file.name }}</span>
          <button class="btn-muted" @click="openFile(file)">Preview</button>
        </article>
      </div>
      <p v-else class="p-10 text-center text-sm text-slate-500">No files uploaded.</p>
      <div v-if="selectedFile" class="border-t border-slate-100 p-5 dark:border-slate-700">
        <div class="flex items-center justify-between mb-3">
          <b class="text-sm">{{ selectedFile.name }}</b>
          <button class="btn-brand text-xs" @click="saveFile">Save file</button>
        </div>
        <textarea v-model="filePreview" class="min-h-48 w-full rounded-lg border border-slate-700 bg-slate-900 p-3 font-mono text-xs text-slate-100 outline-none focus:border-brand"></textarea>
      </div>
    </section>
    ...
    <section v-else class="panel overflow-hidden">
      ...
      <div class="space-y-3 p-5">
        <article v-for="c in comments.slice().reverse()" :key="c.id" class="rounded-xl bg-slate-50 p-4 dark:bg-slate-800/65" :class="c.parentId ? 'ml-8 border-l-2 border-brand/30' : ''">
          <b>{{ store.user(c.userId)?.name || "Team member" }}</b>
          <p class="mt-1 text-sm">{{ c.text }}</p>
          <small class="mt-2 block text-slate-400">{{ store.date(c.at) }}</small>
          <button class="mt-2 text-xs font-bold text-brand hover:underline" @click="replyTo = c">Reply</button>
        </article>
        <p v-if="!comments.length" class="py-5 text-center text-sm text-slate-500">Start the project conversation.</p>
      </div>
      <form class="border-t border-slate-100 p-5 dark:border-slate-700" @submit.prevent="addComment">
        <div v-if="replyTo" class="mb-2 flex items-center justify-between rounded-lg bg-brand-50 px-3 py-2 text-xs text-brand dark:bg-brand/15">
          <span>Replying to {{ store.user(replyTo.userId)?.name }}</span>
          <button type="button" @click="replyTo = null"><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div class="flex gap-3">
          <input v-model.trim="commentDraft" class="field" :placeholder="replyTo ? 'Write a reply' : 'Write an update'" />
          <button class="btn-brand">{{ replyTo ? "Reply" : "Comment" }}</button>
        </div>
      </form>
    </section>
  </section>
  ...
  <section v-else class="panel p-8 text-center">
    <h1 class="text-2xl font-800">Project not found</h1>
    <RouterLink to="/projects" class="btn-brand mt-4">Back to projects</RouterLink>
  </section>

  <Teleport to="body">
    <Transition name="toast">
      <div v-if="focusedMilestone" class="fixed inset-0 z-60 grid place-items-center p-4" role="dialog" aria-modal="true" aria-label="Milestone tasks">
        <button class="absolute inset-0 bg-slate-950/60 backdrop-blur-sm" aria-label="Close task view" @click="closeMilestone"></button>
        <section class="glass relative max-h-[85vh] w-full max-w-3xl overflow-auto rounded-2xl p-6 shadow-2xl">
          <div class="flex items-start justify-between gap-4">
            <div><p class="premium-kicker text-xs font-bold uppercase text-brand">Milestone tasks</p><h2 class="premium-title mt-2 text-2xl font-800">{{ focusedMilestone.title }}</h2></div>
            <button class="btn-muted h-9 w-9 p-0" aria-label="Close" @click="closeMilestone"><i class="fa-solid fa-xmark"></i></button>
          </div>
          <div v-if="focusedTasks.length" class="mt-6 grid gap-3 md:grid-cols-2">
            <article v-for="task in focusedTasks" :key="task.id" class="rounded-xl border border-slate-200 bg-white/60 p-4 dark:border-slate-700 dark:bg-slate-800/50">
              ...
              <div class="mt-4 flex items-center justify-between text-xs text-slate-500">
                <span><i class="fa-regular fa-comment-dots mr-1"></i>{{ task.commentsCount || 0 }}</span>
                <span><i class="fa-solid fa-paperclip mr-1"></i>{{ task.attachmentsCount || 0 }}</span>
                <span>{{ task.assignedTo?.map((id) => store.user(id)?.avatar).join(" · ") }}</span>
              </div>
            </article>
          </div>
          <p v-else class="mt-6 rounded-xl bg-slate-50 p-8 text-center text-sm text-slate-500 dark:bg-slate-800/60">No tasks have been defined for this milestone.</p>
        </section>
      </div>
    </Transition>
  </Teleport>
