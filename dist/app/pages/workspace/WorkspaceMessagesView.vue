<template>
  <div class="grid min-h-[60vh] gap-4 lg:grid-cols-[.8fr_1.2fr]">
    <div class="panel overflow-hidden divide-y divide-slate-100 dark:divide-slate-700">
      ...
      >
        <p class="font-bold">{{ otherParticipant(conv)?.name || "Project team" }}</p>
        <p class="mt-1 truncate text-sm text-brand">{{ jobName(conv.jobId) }}</p>
    ...
    <div class="panel flex min-h-[60vh] flex-col overflow-hidden">
      <template v-if="activeConversation">
        <div class="border-b border-slate-100 p-5 dark:border-slate-700">
          <div class="flex items-center gap-3">
            <span class="grid h-9 w-9 place-items-center rounded-full bg-brand-50 text-xs font-bold text-brand dark:bg-brand/20">
              {{ otherParticipant(activeConversation)?.avatar || "PT" }}
            </span>
            <div>
              <h2 class="font-bold">{{ otherParticipant(activeConversation)?.name || "Project team" }}</h2>
              <RouterLink :to="`/job/${activeConversation.jobId}`" class="mt-0.5 block text-xs font-semibold text-brand hover:underline">
                {{ jobName(activeConversation.jobId) }}
              </RouterLink>
            </div>
          </div>
        </div>
        <div class="flex-1 space-y-3 overflow-auto bg-slate-50 p-5 dark:bg-slate-800/40">
          <div
          ...
          >
            <p>{{ msg.text }}</p>
            <small class="mt-1 block opacity-70">{{ formatDate(msg.at) }}</small>
          </div>
        </div>
        <form class="flex gap-3 border-t border-slate-100 p-4 dark:border-slate-700" @submit.prevent="$emit('send-message')">
          <input
            :value="message"
            class="field"
      ...
      <div v-else class="grid flex-1 place-items-center p-8 text-center text-slate-400">
