<template><section v-if="allowed" class="mx-auto max-w-5xl space-y-6"><header><p class="premium-kicker text-[11px] font-bold uppercase tracking-widest text-brand">
      ...
      ><i
        ...
        >{{
      ...
      </p></header><form class="space-y-5" @submit.prevent="save"><article class="panel p-6 rounded-2xl border border-slate-200/80 bg-white/90 shadow-card dark:border-slate-800/80 dark:bg-slate-900/80"><h2 class="font-head font-800 text-sm tracking-tight text-slate-900 dark:text-white">Issuer & receiver</h2><div class="mt-4 grid gap-4 sm:grid-cols-2"><label class="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500"
            >Issuer<select v-model="form.issuerId" class="field mt-1.5 text-xs font-semibold" required><option
          ...
          ></div></article><article class="panel p-6 rounded-2xl border border-slate-200/80 bg-white/90 shadow-card dark:border-slate-800/80 dark:bg-slate-900/80"><h2 class="font-head font-800 text-sm tracking-tight text-slate-900 dark:text-white">
          {{ paymentMode ? "Payment details" : "Fiscal details" }}
        </h2><div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><label class="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500"
            >Currency<select
              v-model="form.currency"
              class="field mt-1.5 text-xs"
              required
            ><option>MXN</option><option>USD</option></select></label
          ><label class="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500"
            >Date<input
              v-model="form.date"
              type="date"
          ...
          ><label class="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500"
            ...
            }}<select v-model="form.method" class="field mt-1.5 text-xs" required><option value="03">03 · Electronic transfer</option><option value="01">01 · Cash</option><option value="G03">G03 · General expenses</option></select></label
          ><label class="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500"
            >Exchange rate<input
              ...
              required
          /></label></div></article><article v-if="!paymentMode" class="panel p-6 rounded-2xl border border-slate-200/80 bg-white/90 shadow-card dark:border-slate-800/80 dark:bg-slate-900/80"><div class="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800"><div><h2 class="font-head font-800 text-sm tracking-tight text-slate-900 dark:text-white">Concepts</h2><p class="mt-0.5 text-xs text-slate-400">
              Amounts and VAT are calculated automatically.
            </p></div><button type="button" class="btn-muted text-xs py-1.5 px-3" @click="addLine"><i class="fa-solid fa-plus mr-1.5 text-xs"></i>Add concept
          </button></div><div class="mt-4 space-y-3"><div
          ...
          ><label
            ...
            ><i class="fa-solid fa-trash text-xs"></i></button></div></div></article><article v-else class="panel p-6"><h2 class="font-800">Related document</h2><div class="mt-4 grid gap-4 sm:grid-cols-3"><label class="text-sm font-semibold sm:col-span-2"
            >Invoice<select
          ...
          ><label class="text-sm font-semibold"
            >Amount applied<input
              ...
              step="0.01"
              required
          /></label></div><p v-if="selectedInvoice" class="mt-4 text-sm text-slate-500">
          Previous balance
      ...
      ><div><p class="text-sm text-slate-500">Total</p><p class="text-3xl font-800">
            {{ store.money(total, form.currency) }}
          </p><p v-if="!paymentMode" class="text-xs text-slate-500">
            Subtotal {{ store.money(subtotal, form.currency) }} · VAT
            {{ store.money(tax, form.currency) }}
          </p></div><div class="flex gap-2"><RouterLink :to="parent" class="btn-muted">Cancel</RouterLink
          ><button class="btn-brand"><i class="fa-solid fa-stamp mr-2"></i
            >{{ editing ? "Update" : "Save & stamp" }}
          </button></div></article></form></section><section v-else class="panel mx-auto max-w-xl p-10 text-center"><i class="fa-solid fa-lock text-3xl text-slate-400"></i><h1 class="mt-4 text-2xl font-800">Fiscal document unavailable</h1><RouterLink :to="parent" class="btn-brand mt-5">Return</RouterLink></section></template>
<script>
