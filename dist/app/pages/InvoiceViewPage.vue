<template><div><section v-if="invoice && allowed" class="space-y-5"><header class="flex flex-wrap items-start justify-between gap-4"><div><p class="premium-kicker text-xs font-bold uppercase text-brand">
            CFDI 4.0
          </p><h1 class="premium-title mt-2 text-3xl font-800">
            Invoice {{ invoice.serie }}-{{ invoice.folio }}
          </h1></div><div class="flex flex-wrap gap-2"><button class="btn-muted" @click="print"><i class="fa-solid fa-print mr-2"></i>Print / PDF</button
          ><button v-if="canManage" class="btn-muted" @click="clone"><i class="fa-regular fa-copy mr-2"></i>Clone</button
          ><RouterLink
            v-if="canManage && invoice.status === 'Vigente'"
            class="btn-brand"
            :to="`/invoices/${invoice.id}/edit`"
            ><i class="fa-solid fa-pen mr-2"></i>Edit</RouterLink
          ><button v-if="canManage" class="btn-muted" @click="send"><i class="fa-regular fa-paper-plane mr-2"></i
      ...
      ><b>Cancelled CFDI</b><p class="mt-1">Motive: {{ invoice.cancellationDetails?.motive }}</p><p v-if="invoice.cancellationDetails?.replacementUuid">
          Replacement: {{ invoice.cancellationDetails.replacementUuid }}
        </p></div><article class="panel overflow-hidden"><div class="border-b border-slate-100 p-6 dark:border-slate-700"><div class="flex flex-wrap justify-between gap-5"><div><p class="text-xs font-bold uppercase text-slate-400">Issuer</p><h2 class="mt-2 text-lg font-800">
                {{ issuer?.name || store.user(invoice.providerId)?.name }}
              </h2><p class="font-mono text-sm text-slate-500">
                {{ issuer?.rfc || "XAXX010101000" }} · Regime
                {{ issuer?.regime || "612" }}
              </p></div><div class="text-right"><span
                class="badge"
                :class="
              ...
              </p></div></div></div><div class="grid gap-5 p-6 md:grid-cols-2"><div><p class="text-xs font-bold uppercase text-slate-400">Receiver</p><b class="mt-2 block">{{ receiver?.name }}</b><p class="text-sm text-slate-500">
              {{ receiver?.rfc || "XAXX010101000" }}
            </p></div><div><p class="text-xs font-bold uppercase text-slate-400">
              Fiscal identifiers
            </p><p class="mt-2 break-all font-mono text-xs">
        ...
        ><table class="w-full text-sm"><thead
            ...
            ><tr><th class="p-4">Concept</th><th class="p-4 text-right">Qty</th><th class="p-4 text-right">Unit price</th><th class="p-4 text-right">Amount</th></tr></thead><tbody><tr
                v-for="line in lines"
                :key="line.id"
                class="border-t border-slate-100 dark:border-slate-700"
              ><td class="p-4">{{ line.description }}</td><td class="p-4 text-right">{{ line.quantity }}</td><td class="p-4 text-right">
                  {{ store.money(line.unitPrice, invoice.currency) }}
                </td><td class="p-4 text-right font-bold">
                  {{ store.money(line.amount, invoice.currency) }}
                </td></tr></tbody></table></div><div
          class="flex justify-end border-t border-slate-100 p-6 dark:border-slate-700"
        ><dl class="w-64 space-y-2 text-sm"><div class="flex justify-between"><dt class="text-slate-500">Subtotal</dt><dd>{{ store.money(subtotal, invoice.currency) }}</dd></div><div class="flex justify-between"><dt class="text-slate-500">VAT</dt><dd>{{ store.money(tax, invoice.currency) }}</dd></div><div class="flex justify-between border-t pt-2 text-lg font-800"><dt>Total</dt><dd>{{ store.money(invoice.total, invoice.currency) }}</dd></div></dl></div></article><div
        v-if="cancelOpen"
      ...
      ><form
        ...
        ><h2 class="text-xl font-800">Cancel invoice</h2><label class="block text-sm font-semibold"
            >SAT motive<select
            ...
            ><option value="01">
              ...
              </option><option value="03">03 · Operation did not take place</option><option value="04">
          ...
          ><label
            ...
            >Replacement UUID<input
              ...
              class="field mt-2"
              required
          /></label><div class="flex justify-end gap-2"><button type="button" class="btn-muted" @click="cancelOpen = false">
              Keep invoice</button
            ><button
              class="rounded-lg bg-rose-600 px-4 py-2 text-sm font-semibold text-white"
            >
              Confirm cancellation
            </button></div></form></div></section><section v-else class="panel p-10 text-center"><i class="fa-solid fa-lock text-3xl text-slate-400"></i><h1 class="mt-4 text-2xl font-800">Invoice unavailable</h1><RouterLink to="/invoices" class="btn-brand mt-4"
