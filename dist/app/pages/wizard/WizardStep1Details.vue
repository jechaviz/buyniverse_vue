<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-4">
      <div>
        <p class="premium-kicker text-[11px] font-bold uppercase tracking-widest text-brand">{{ store.t("Core brief") }}</p>
        <h2 class="font-head mt-0.5 text-2xl font-800 tracking-tight text-slate-900 dark:text-white">{{ store.t("Project details") }}</h2>
        <p class="mt-1 text-xs sm:text-sm text-slate-500 dark:text-slate-400">
          {{ store.t("Define the information providers need to qualify and price the work.") }}
        </p>
      </div>
      <!-- Sourcing Type Switcher -->
      <div class="flex items-center gap-1.5 self-start rounded-xl border border-slate-200/80 bg-slate-50/80 p-1 dark:border-slate-700 dark:bg-slate-800/80">
        <button
          type="button"
    ...
    <!-- AI Brief Assistant Panel (Collapsible / Integrated) -->
    <div class="rounded-2xl border border-brand/25 bg-brand-50/40 p-4.5 dark:bg-brand/10 shadow-xs">
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="flex items-center gap-2">
          <span class="grid h-7 w-7 place-items-center rounded-lg bg-brand text-white text-xs shadow-xs">
            <i class="fa-solid fa-wand-magic-sparkles"></i>
          </span>
          <b class="text-xs font-800 text-slate-900 dark:text-white">{{ store.t("Project assistant") }}</b>
          <span class="text-[11px] text-slate-500 dark:text-slate-400 hidden sm:inline">— {{ store.t("Describe the outcome") }}</span>
        </div>
    ...
    <!-- Main Project Description Form Fields -->
    <div class="grid gap-5 md:grid-cols-2">
      ...
      <label class="block text-xs font-bold">
        ...
        <select v-model="project.category" class="field" required>
          ...
          <option value="Marketing">{{ store.t("Marketing") }}</option>
          <option value="Operations">{{ store.t("Operations") }}</option>
        </select>
      </label>
      ...
      <label class="block text-xs font-bold">
        ...
        <select v-model="project.projectLevel" class="field" required>
          ...
          <option value="Expert">{{ store.t("Expert") }}</option>
        </select>
      </label>

      <!-- Budget & Currency (for RFP) -->
      <template v-if="project.sourcingType === 'RFP'">
        ...
        <label class="block text-xs font-bold">
          ...
          <select v-model="project.duration" class="field" required>
            ...
            <option value="3 to 6 months">{{ store.t("3 to 6 months") }}</option>
            <option value="More than 6 months">{{ store.t("More than 6 months") }}</option>
          </select>
        </label>
        ...
        <label class="block text-xs font-bold">
          <span class="mb-1.5 block">{{ store.t("Visibility") }}</span>
          <select v-model="project.visibility" class="field">
            <option value="private">{{ store.t("Private") }}</option>
            <option value="public">{{ store.t("Public") }}</option>
          </select>
        </label>

        <!-- Skills with tags -->
        <div class="block text-xs font-bold md:col-span-2">
          <div class="flex items-center justify-between mb-1.5">
            <span>{{ store.t("Skills") }} <span class="text-[10px] font-normal text-slate-400">({{ store.t("comma separated") }})</span></span>
...
const DocumentEditorModal = defineAsyncComponent(() =>
    window["vue3-sfc-loader"].loadModule("./app/components/document/DocumentEditorModal.vue?v=13", window.sfcOptions)
);
