<template><section class="space-y-6"><div class="flex flex-wrap items-center justify-between gap-3"><div><h1 class="premium-title mt-1 text-3xl font-bold">{{ config.title }}</h1><p class="mt-2 text-slate-500">{{ config.copy }}</p></div><button v-if="config.action && config.action.mode === 'invoice'" class="btn-brand" @click="openNew"><i class="fa-solid fa-plus mr-2"></i>{{ config.action.label }}
      </button><RouterLink v-else-if="config.action" :to="config.action.to" class="btn-brand"><i class="fa-solid fa-plus mr-2"></i>{{ config.action.label }}
      </RouterLink></div><WorkspaceMessagesView
      v-if="kind === 'messages'"
    ...
    /><WorkspaceNewInvoiceModal :open="newInvoiceOpen" :draft="invoiceDraft" @close="newInvoiceOpen = false" @create="createInvoice" /></section></template>
<script>
