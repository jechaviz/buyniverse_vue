<template><section class="space-y-6"><header class="flex flex-wrap items-center justify-between gap-4"><div><p class="premium-kicker text-[11px] font-bold uppercase tracking-widest text-brand">
          Dashboard
        </p><h1 class="font-head mt-1 text-2xl sm:text-3xl font-800 tracking-tight text-slate-900 dark:text-white">
          {{ titles[section] }}
        </h1><p class="mt-1 text-xs sm:text-sm text-slate-500 dark:text-slate-400">{{ copies[section] }}</p></div><RouterLink
      ...
      ></header><nav class="panel flex gap-1.5 overflow-x-auto p-1.5 bg-slate-100/80 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl"><RouterLink
        v-for="x in tabs"
        :key="x.key"
        :to="x.to"
        ...
        >{{ x.label }}</RouterLink
      ></nav><template v-if="section === 'overview'"><div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><article
          v-for="card in cards"
          :key="card.label"
          class="premium-card rounded-2xl p-5 border border-slate-200/80 dark:border-slate-800/80 bg-white/90 dark:bg-slate-900/80"
        ><div class="flex items-center justify-between"><p class="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">{{ card.label }}</p><span class="grid h-8 w-8 place-items-center rounded-xl bg-brand-50 text-brand dark:bg-brand/15 text-xs"><i class="fa-solid" :class="card.icon || 'fa-chart-simple'"></i></span></div><p class="font-head mt-3 text-2xl sm:text-3xl font-800 tracking-tight text-slate-900 dark:text-white">{{ card.value }}</p><p class="mt-1.5 text-[11px] font-medium text-slate-400 dark:text-slate-400 flex items-center gap-1.5"><span class="inline-block h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
            {{ card.note }}
          </p></article></div><AdminDatabaseCard v-if="store.isAdmin.value || user.type === 'Admin'" /><article
      ...
      ><div><div class="flex items-center justify-between gap-3"><div><p
                class="premium-kicker text-[10px] font-800 uppercase tracking-wider text-brand"
              >
                Continue working
              </p><h2 class="font-head mt-0.5 font-800 text-base text-slate-900 dark:text-white">Recent workspaces</h2></div><span class="text-[10px] font-semibold text-slate-400 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-full"
              ><i class="fa-solid fa-shield-halved mr-1 text-emerald-500"></i>15
              min privacy lock</span
            ></div><div v-if="recent.length" class="mt-3.5 flex flex-wrap gap-2"><RouterLink
            ...
            ></div><p v-else class="mt-3 text-xs text-slate-500 dark:text-slate-400">
            Open a project, invoice or request and it will appear here.
          </p></div><div class="flex flex-wrap gap-2 lg:justify-end"><RouterLink
          ...
          ></div></article><div class="grid gap-6 lg:grid-cols-2"><article class="panel p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80"><div class="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800"><h2 class="font-head font-800 text-sm tracking-tight text-slate-900 dark:text-white">Active contracts</h2><span class="rounded-full bg-slate-100 dark:bg-slate-800 px-2.5 py-0.5 text-[10px] font-bold text-slate-500">{{ contracts.length }} active</span></div><div class="mt-4 space-y-2.5"><RouterLink
              ...
              ><span class="min-w-0 pr-3"
                ...
                ><span class="mt-1 inline-flex items-center gap-1.5 text-[11px] text-slate-400"><span class="h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
                  {{ x.status }}
                </span></span
              ><b class="text-xs font-mono font-bold text-slate-800 dark:text-slate-200">{{ store.money(x.amount) }}</b></RouterLink
            ><p v-if="!contracts.length" class="p-6 text-center text-xs text-slate-400">No active contracts.</p></div></article><article class="panel p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80"><div class="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800"><h2 class="font-head font-800 text-sm tracking-tight text-slate-900 dark:text-white">Recent activity</h2><RouterLink to="/dashboard/transactions" class="text-xs font-semibold text-brand hover:underline">View all</RouterLink></div><div class="mt-4 space-y-2.5"><div
            ...
            ><div class="min-w-0 pr-3"><span class="block truncate font-semibold text-slate-800 dark:text-slate-200">{{ x.description }}</span><span class="text-[10px] text-slate-400">{{ store.date(x.date) }}</span></div><b class="font-mono font-bold" :class="x.amount >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-700 dark:text-slate-300'">{{ store.money(x.amount) }}</b></div><p v-if="!transactions.length" class="p-6 text-center text-xs text-slate-400">No recent activity.</p></div></article></div></template
    ><article v-else-if="section === 'timesheets'" class="panel overflow-hidden rounded-2xl border border-slate-200/80 dark:border-slate-800/80"><div
      ...
      ><span>Date</span><span>Work</span><span>Project</span><span class="text-right">Hours</span></div><div
        v-for="x in times"
        :key="x.id"
      ...
      ><span class="text-slate-500 font-medium">{{ store.date(x.date) }}</span
        ...
        ><RouterLink class="text-brand font-semibold hover:underline" :to="`/contract/${x.contractId}`"
          >Contract</RouterLink
        ><b class="text-right font-mono font-bold">{{ x.hours }}</b></div><p v-if="!times.length" class="p-8 text-center text-xs text-slate-400">
        No time entries.
      </p></article><article
    ...
    ><div
      ...
      ><div><b class="font-bold text-slate-800 dark:text-slate-100">{{ x.description }}</b><p class="mt-0.5 text-[11px] text-slate-400">
            {{ x.type }} · {{ store.date(x.date) }}
          </p></div><b class="font-mono text-sm font-bold" :class="x.amount >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-500'">{{
          store.money(x.amount)
        }}</b></div><p v-if="!transactions.length" class="p-8 text-center text-xs text-slate-400">
        No transactions.
      </p></article><article v-else-if="section === 'my-agency'" class="panel p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80"><template v-if="agency"
        ><div class="flex flex-wrap items-center justify-between gap-4"><div><p class="text-xs font-bold uppercase tracking-wider text-brand">Agency workspace</p><h2 class="font-head mt-1 text-2xl font-800 tracking-tight text-slate-900 dark:text-white">{{ agency.name }}</h2><p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ agency.tagline }}</p></div><RouterLink class="btn-brand text-xs py-2 px-4" :to="`/agency/${agency.id}`"
            >Manage agency</RouterLink
          ></div><div class="mt-6 flex flex-wrap gap-2"><span
