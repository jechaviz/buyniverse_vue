<template>
  ...
  <section v-else class="space-y-6">
    ...
    <header class="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-200/80 dark:border-slate-800/80">
      <div class="space-y-1">
        <div class="inline-flex items-center gap-2 rounded-full border border-brand-200/70 bg-brand-50/70 px-2.5 py-0.5 text-[10px] font-800 uppercase tracking-widest text-brand dark:border-brand-500/30 dark:bg-brand-500/10 dark:text-brand-300">
      ...
      <div class="flex items-center gap-2.5">
        <RouterLink
          ...
          class="btn-brand text-xs sm:text-sm py-2.5 px-5 font-bold shadow-sm inline-flex items-center active:scale-98"
          to="/post-job/new"
        >
          <i class="fa-solid fa-plus mr-2"></i>{{ store.t("Post a job") }}
        </RouterLink>
        <RouterLink
          ...
          to="/find-work"
        >
          <i class="fa-solid fa-briefcase mr-2"></i>{{ store.t("Find work") }}
        </RouterLink>
      </div>
    </header>

    <!-- Navigation Tabs -->
    <nav class="flex gap-1.5 overflow-x-auto p-1.5 bg-slate-100/80 dark:bg-slate-900/80 border border-slate-200/90 dark:border-slate-800/80 rounded-2xl backdrop-blur-md">
      <RouterLink
        v-for="x in tabs"
        :key="x.key"
        :to="x.to"
        class="whitespace-nowrap rounded-xl px-4 py-2 text-xs font-bold transition-all active:scale-98"
        :class="section === x.key ? 'bg-white text-brand shadow-sm dark:bg-slate-800 dark:text-brand-300' : 'text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200'"
      >
        {{ store.t(x.label) }}
      </RouterLink>
    </nav>
    ...
    <template v-if="section === 'overview'">
      ...
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <article
          v-for="card in cards"
          :key="card.label"
          class="panel p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white/90 dark:bg-slate-900/80 transition-all hover:border-brand/30 hover:shadow-card-elevated"
        >
          <div class="flex items-center justify-between">
            <p class="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">{{ store.t(card.label) }}</p>
            <span class="grid h-8 w-8 place-items-center rounded-xl bg-brand-50 text-brand dark:bg-brand/15 text-xs shadow-xs">
              <i class="fa-solid" :class="card.icon || 'fa-chart-simple'"></i>
            </span>
          </div>
          <p class="font-head font-mono mt-3 text-2xl sm:text-3xl font-800 tracking-tight text-slate-900 dark:text-white">{{ card.value }}</p>
      ...
      <!-- Recent Workspaces & Quick Actions Bar -->
      <article
      ...
      >
        <div>
          <div class="flex items-center justify-between gap-3">
            <div>
              <p class="text-[10px] font-800 uppercase tracking-wider text-brand">
            ...
            <span class="text-[10px] font-semibold text-slate-400 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-full">
              <i class="fa-solid fa-shield-halved mr-1 text-emerald-500"></i>15 {{ store.t("min privacy lock") }}
            </span>
          </div>
          <div v-if="recent.length" class="mt-3.5 flex flex-wrap gap-2">
            ...
            >
              <i class="fa-solid fa-clock-rotate-left mr-2 text-[10px] text-slate-400"></i>
              {{ store.t(item.label) }}
            </RouterLink>
          </div>
        ...
        <div class="flex flex-wrap gap-2 lg:justify-end">
          <RouterLink
            ...
            :key="action.to"
            :to="action.to"
            class="btn-muted whitespace-nowrap text-xs py-2 px-3.5 shadow-xs"
          >
            <i class="fa-solid mr-1.5 text-xs" :class="action.icon"></i>
            {{ store.t(action.label) }}
          </RouterLink>
        </div>
      </article>

      <!-- Active Contracts & Recent Activity Grids -->
        ...
        <article class="panel p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80">
          ...
          <div class="mt-4 space-y-2.5">
            ...
            >
              ...
              <b class="text-xs font-mono font-bold text-slate-800 dark:text-slate-200">{{ store.money(x.amount) }}</b>
            </RouterLink>
    ...
    <article v-else-if="section === 'timesheets' && store.isSupplier.value" class="panel overflow-hidden rounded-2xl border border-slate-200/80 dark:border-slate-800/80">
      <div class="grid grid-cols-[8rem_1fr_9rem_5rem] gap-3 bg-slate-50/80 p-4 text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:bg-slate-800/80 border-b border-slate-200/80 dark:border-slate-800">
      ...
      >
        ...
        <RouterLink class="text-brand font-semibold hover:underline" :to="`/contract/${x.contractId}`">{{ store.t("Contract") }}</RouterLink>
    ...
    <article v-else-if="section === 'my-agency' && store.isSupplier.value" class="panel p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80">
      <template v-if="agency">
        <div class="flex flex-wrap items-center justify-between gap-4">
          ...
          <RouterLink class="btn-brand text-xs py-2 px-4" :to="`/agency/${agency.id}`">{{ store.t("Manage agency") }}</RouterLink>
        </div>
        <div class="mt-6 flex flex-wrap gap-2">
          ...
          >
            {{ store.user(m.userId)?.name }} · {{ store.t(m.role) }}
          </span>
        </div>
      </template>
