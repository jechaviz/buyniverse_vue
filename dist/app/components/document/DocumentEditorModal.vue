<template>
  ...
  >
    ...
    >
      ...
      <header class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200/90 bg-white px-5 py-3.5 dark:border-slate-800 dark:bg-slate-900/95 flex-none">
        <div class="flex items-center gap-3 min-w-0">
          <span class="grid h-9 w-9 place-items-center rounded-xl bg-brand text-white text-sm shadow-soft">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <div class="relative">
            ...
            >
              ...
              >
                <b class="text-slate-900 dark:text-white block font-bold">{{ tpl.name }}</b>
                <span class="text-[11px] text-slate-400 block mt-0.5">{{ tpl.desc }}</span>
              </button>
            </div>
          </div>
          ...
          >
            ...
            <span>{{ store.t("Insertar en Descripción") }}</span>
          </button>
      ...
      <!-- Optional Header / Footer Configuration Drawer -->
      <div
      ...
      >
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 max-w-6xl mx-auto text-xs">
          <div>
            <label class="font-bold text-slate-700 dark:text-slate-200 block mb-1">
          ...
          <div>
            ...
            <select v-model="pageNumberFormat" class="field text-xs py-1.5">
              <option value="Page X of Y">{{ store.t("Página X de Y") }}</option>
              <option value="X / Y">X / Y</option>
              <option value="Page X">{{ store.t("Página X") }}</option>
              <option value="none">{{ store.t("Sin numeración") }}</option>
            </select>
          </div>
          <div class="flex items-end">
            <label class="flex items-center gap-2 cursor-pointer pb-2">
              <input v-model="showRunningHeader" type="checkbox" class="accent-brand" />
              <span class="font-bold text-slate-700 dark:text-slate-200">{{ store.t("Mostrar en todas las páginas") }}</span>
            </label>
          </div>
        </div>
      </div>

      <!-- Main Dual Panel Canvas -->
      <div class="flex flex-1 min-h-0 overflow-hidden">
        <!-- =================================================================== -->
        <!-- LEFT PANEL: Letter-sized Page Thumbnails & Section Hierarchy Tree  -->
        <!-- =================================================================== -->
              ...
              >
                ...
                <div class="flex-1 overflow-hidden py-2 space-y-2 pointer-events-none">
                  <div
                    ...
                    class="space-y-1"
                  >
                    <!-- Hierarchical Section Header in Miniature -->
                    ...
                    <!-- Miniature text simulation lines -->
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Auto-computes hierarchical numbers: 1, 1.1, 1.1.1, 1.2, 2, 2.1...
    ...
    // Estimated page builder for Letter format thumbnails
    const estimatedPages = computed(() => {
      ...
      sections.value.forEach((sec, idx) => {
        if (sec.pageBreakBefore && currentPage.sections.length > 0) {
          ...
          // If content exceeds ~250 words, create virtual page
    ...
    // Structured blocks parser for live preview (safe DOM rendering)
    ...
    // Compile entire structured tree to Clean Markdown
