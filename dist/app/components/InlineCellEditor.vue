<template>
  <div ref="root" class="relative min-w-28" @keydown.stop>
    ...
    <select v-else-if="type==='select'||type==='user'" ref="input" v-model="draft" class="field py-1.5 text-sm" @change="save" @keydown.esc.prevent="cancel" @blur="save"><option v-if="type==='user'" value="">{{ t('Unassigned') }}</option><option v-for="option in resolvedOptions" :key="option.value" :value="option.value">{{ t(option.label) }}</option></select>
    ...
    <div v-else-if="type==='rating'" class="flex gap-0.5"><button v-for="star in 5" :key="star" class="text-lg" :class="star<=Number(draft)?'text-amber-400':'text-slate-300'" :aria-label="t(`Rate ${star} stars`)" @click="draft=star;save()"><i class="fa-solid fa-star"></i></button></div>
    <div v-else-if="type==='tags'" class="relative"><input ref="input" v-model="draft" class="field py-1.5 text-sm" :placeholder="t('tag, tag')" maxlength="240" @keydown.enter.prevent="saveTags" @keydown.esc.prevent="cancel" @blur="saveTags"/><p class="mt-1 text-[10px] text-slate-400">{{ t('Separate with commas') }}</p></div>
    <div v-else-if="type==='multi-user'" class="glass absolute left-0 top-0 z-30 w-56 rounded-xl p-2 shadow-xl"><p class="px-2 py-1 text-xs font-bold text-slate-400">{{ t('Assign people') }}</p><label v-for="person in users" :key="person.id" class="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-700"><input v-model="draftUsers" type="checkbox" :value="person.id"/><span class="grid h-6 w-6 place-items-center rounded-full bg-brand-50 text-[10px] font-bold text-brand">{{ person.avatar||person.name?.slice(0,2) }}</span><span class="truncate">{{ person.name }}</span></label><div class="mt-2 flex justify-end gap-2 border-t border-slate-100 pt-2 dark:border-slate-700"><button class="text-xs text-slate-500" @click="cancel">{{ t('Cancel') }}</button><button class="text-xs font-semibold text-brand" @click="saveUsers">{{ t('Save') }}</button></div></div>
  ...
  methods: {
    ...
    save() {
      ...
      if (this.type === 'number' || this.type === 'slider' || this.type === 'rating') {
        value = Number(value);
        if (!Number.isFinite(value)) return this.invalid(this.t('Enter a valid number.'));
        ...
        if (value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) return this.invalid(this.t('Enter a valid email address.'));
      } else if (this.type === 'date') {
        value = String(value || '').slice(0, 10);
        if (value && !/^\d{4}-\d{2}-\d{2}$/.test(value)) return this.invalid(this.t('Enter a valid date.'));
      } else if (this.type === 'select' || this.type === 'user') {
        if (!this.resolvedOptions.some(option => String(option.value) === String(value)) && value !== '') return this.invalid(this.t('Choose an available option.'));
