<template><div
  ...
  ><span v-if="!actions.length" class="text-slate-300">—</span><button
      v-else
      type="button"
    ...
    ><i class="fa-solid text-xs" :class="primaryAction.icon"></i></button><div
      v-if="actions.length"
    ...
    ><button
        ...
        @click="run(action)"
      ><i class="fa-solid text-xs" :class="action.icon"></i></button></div></div></template>
<script>
export default {
props: {
