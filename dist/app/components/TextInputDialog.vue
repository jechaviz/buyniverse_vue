      ...
      ><button
        ...
        ><header
          ...
          ><span
            ...
            ><i class="fa-solid" :class="icon"></i></span><div class="min-w-0 flex-1"><h2 :id="`${dialogId}-title`" class="text-lg font-800">
                {{ title }}
              </h2><p
                v-if="description"
              ...
              aria-label="Close dialog"
              @click="close"
            ><i class="fa-solid fa-xmark"></i></button></header><div class="p-5"><label
              :for="`${dialogId}-input`"
              class="mb-2 flex items-center gap-2"
            ><span class="text-xs font-bold">{{ label }}</span><span
                v-if="required"
            ...
            ><p
              ...
              ><i class="fa-solid fa-circle-exclamation mr-1"></i
                >{{ validationError }}
              </p><p v-else class="text-slate-400">{{ hint }}</p><span class="ml-auto whitespace-nowrap text-slate-400"
          ...
          ><button type="button" class="btn-muted" @click="close">
              Cancel
            </button><button type="submit" class="btn-brand" :disabled="!canSubmit"><i class="fa-solid fa-check"></i>{{ confirmLabel }}
            </button></footer></form></div></Transition></Teleport></template>
<script>
const { nextTick } = Vue;
export default {
props: {
