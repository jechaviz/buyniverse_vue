<template><div
  ...
  ><div class="flex min-w-0 items-center gap-3"><span
      ...
      ><i class="fa-solid fa-check-double"></i></span><span class="whitespace-nowrap"
      ...
      </button><span v-else class="hidden text-xs text-slate-500 sm:block"
    ...
    ><button
      ...
      ><i class="fa-solid fa-bolt"></i>Bulk actions
        ...
        >
          {{ actions.length }}
        </span><i class="fa-solid fa-chevron-down text-[8px] opacity-60"></i></button><button
        type="button"
      ...
      ><p
        ...
        >
        ...
        </p><template v-for="(action, index) in actions" :key="action.key"><div
            v-if="
              action.tone === 'danger' &&
              index > 0 &&
          ...
          ><span
            ...
            ><i class="fa-solid text-xs" :class="action.icon"></i></span><span class="min-w-0 flex-1"><b class="block truncate text-xs">{{ action.label }}</b><small
                v-if="action.description"
                class="mt-0.5 block truncate text-[9px] opacity-65"
              >
                {{ action.description }}
              </small></span></button></template>
<script>
export default {
props: {
