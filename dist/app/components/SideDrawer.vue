      ...
      ><button
        ...
        ><header
          ...
          ><span
            ...
            ><i class="fa-solid" :class="icon"></i></span><div class="min-w-0 flex-1"><h2 :id="`${drawerId}-title`" class="text-lg font-800">
                {{ title }}
              </h2><p v-if="subtitle" class="mt-1 text-xs leading-5 text-slate-500">
                {{ subtitle }}
              </p></div><button
              type="button"
              ...
              aria-label="Close drawer"
              @click="$emit('close')"
            ><i class="fa-solid fa-xmark"></i></button></header><div class="min-h-0 flex-1 overflow-y-auto p-5"><slot></slot></div><footer
            v-if="$slots.footer"
          ...
          ><slot name="footer"></slot></footer></aside></div></Transition></Teleport></template>
<script>
export default {
props: {
open: Boolean,
