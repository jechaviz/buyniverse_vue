<template><div class="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3"><article
      v-for="item in items"
      :key="item.id"
      class="rounded-xl border border-slate-200/70 p-4 transition hover:border-brand dark:border-slate-700"
    ><div class="flex items-start justify-between gap-3"><div><b class="text-sm">{{ display(item, visibleColumns[0]?.key) }}</b><p v-if="visibleColumns[1]" class="mt-1 text-xs text-slate-500">
        ...
        /></div><div class="mt-3 flex flex-wrap gap-2 text-xs"><span
