<template><section class="premium-card overflow-hidden rounded-2xl border border-slate-200/80 shadow-card dark:border-slate-800/80 dark:bg-slate-900/90"><div class="flex items-center justify-between gap-3 border-b border-slate-200/70 p-3 sm:p-4 dark:border-slate-800 flex-wrap"><div class="flex min-w-0 flex-1 items-center gap-2 flex-wrap"><div
        ...
        ><button v-if="!searchOpen && !query" type="button" class="btn-muted h-9 w-9 p-0" aria-label="Search records" title="Search" @click="openSearch"><i class="fa-solid fa-search"></i></button><template v-else><i class="fa-solid fa-search pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"></i><input ref="searchInput" v-model="query" class="field h-9 pl-9 pr-9" :placeholder="`Search ${title.toLowerCase()}…`" @blur="closeSearchIfIdle" @keydown.esc="closeSearch" /><button type="button" class="absolute right-1 grid h-7 w-7 place-items-center rounded-md text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-700 dark:hover:text-white" aria-label="Close search" title="Close search" @click="closeSearch"><i class="fa-solid fa-xmark text-xs"></i></button></template>
<script>
