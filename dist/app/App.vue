<template><div class="premium-shell relative min-h-screen bg-slate-50 text-slate-800 dark:bg-slate-950 dark:text-slate-100"><CommandPalette :open="commandOpen" @close="commandOpen = false" /><AppModals :ui="ui" @resume-session="resumeSession" @resolve-confirm="store.resolveConfirm" /><AuthModal :open="authOpen" :initial-mode="authMode" @close="authOpen = false" /><div v-if="isLanding" class="flex-1 flex flex-col min-h-screen"><nav class="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 px-4 py-3.5 backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-950/90 sm:px-8"><div class="max-w-7xl mx-auto flex items-center justify-between gap-4"><RouterLink to="/" class="flex items-center gap-3 group"><span class="grid h-10 w-10 place-items-center rounded-2xl bg-brand font-head text-lg font-bold text-white shadow-md shadow-brand/30 group-hover:scale-105 transition-transform">
              B
            </span><div><span class="font-head text-lg font-800 tracking-tight text-slate-900 dark:text-white block">Buyniverse</span><span class="text-[10px] font-bold uppercase tracking-wider text-brand -mt-1 block">{{ store.t("B2B Sourcing & Talent") }}</span></div></RouterLink><div class="hidden md:flex items-center gap-6 text-xs font-bold text-slate-600 dark:text-slate-300"><RouterLink to="/procurement/auction" class="hover:text-brand transition flex items-center gap-1.5"><span class="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></span>
              {{ store.t("Subastas en Vivo") }}
            </RouterLink><RouterLink to="/procurement/sourcing" class="hover:text-brand transition">
              {{ store.t("Rondas RFX") }}
            </RouterLink><RouterLink to="/find-talent" class="hover:text-brand transition">
              {{ store.t("Buscar Talento") }}
            </RouterLink><RouterLink to="/browse-services" class="hover:text-brand transition">
              {{ store.t("Servicios") }}
            </RouterLink></div><div class="flex items-center gap-2.5 sm:gap-3"><div class="flex rounded-xl bg-slate-100 p-0.5 text-[10px] font-bold dark:bg-slate-800/80" role="group" aria-label="Select language"><button
                v-for="code in ['es', 'en']"
                :key="code"
              ...
              @click="toggleTheme"
            ><i class="fa-solid text-xs" :class="dark ? 'fa-sun' : 'fa-moon'"></i></button><button
              type="button"
            ...
            </RouterLink></div></div></nav><main class="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12"><RouterView :key="route.path" /></main><footer class="border-t border-slate-200/80 bg-white/90 dark:border-slate-800/80 dark:bg-slate-950/90 py-12 px-4 sm:px-8"><div class="max-w-7xl mx-auto grid gap-8 sm:grid-cols-2 lg:grid-cols-5 text-xs"><div class="lg:col-span-2 space-y-3"><div class="flex items-center gap-2.5"><span class="grid h-8 w-8 place-items-center rounded-xl bg-brand font-head font-bold text-white text-sm">B</span><span class="font-head text-base font-800 text-slate-900 dark:text-white">Buyniverse</span></div><p class="text-slate-500 dark:text-slate-400 max-w-sm leading-relaxed">
            ...
            </p></div><div class="space-y-2.5"><b class="font-head text-slate-900 dark:text-white uppercase tracking-wider text-[11px] block">{{ store.t("Soluciones") }}</b><ul class="space-y-1.5 text-slate-500 dark:text-slate-400"><li><RouterLink to="/procurement/auction" class="hover:text-brand">{{ store.t("Subastas Inversas") }}</RouterLink></li><li><RouterLink to="/procurement/sourcing" class="hover:text-brand">{{ store.t("Rondas de Cotización RFX") }}</RouterLink></li><li><RouterLink to="/post-job/new" class="hover:text-brand">{{ store.t("Publicar Proyecto") }}</RouterLink></li><li><RouterLink to="/find-talent" class="hover:text-brand">{{ store.t("Directorio de Proveedores") }}</RouterLink></li></ul></div><div class="space-y-2.5"><b class="font-head text-slate-900 dark:text-white uppercase tracking-wider text-[11px] block">{{ store.t("Cumplimiento") }}</b><ul class="space-y-1.5 text-slate-500 dark:text-slate-400"><li><RouterLink to="/invoices" class="hover:text-brand">{{ store.t("Facturación CFDI 4.0") }}</RouterLink></li><li><RouterLink to="/contracts" class="hover:text-brand">{{ store.t("Fideicomiso por Hitos") }}</RouterLink></li><li><RouterLink to="/dashboard" class="hover:text-brand">{{ store.t("Auditoría Inmutable") }}</RouterLink></li></ul></div><div class="space-y-2.5"><b class="font-head text-slate-900 dark:text-white uppercase tracking-wider text-[11px] block">{{ store.t("Empresa") }}</b><ul class="space-y-1.5 text-slate-500 dark:text-slate-400"><li><a href="#" class="hover:text-brand">{{ store.t("Términos de Servicio") }}</a></li><li><a href="#" class="hover:text-brand">{{ store.t("Aviso de Privacidad") }}</a></li><li><a href="#" class="hover:text-brand">{{ store.t("Soporte Empresarial") }}</a></li></ul></div></div><div class="max-w-7xl mx-auto mt-8 pt-6 border-t border-slate-100 dark:border-slate-900 flex flex-wrap items-center justify-between gap-4 text-[11px] text-slate-400"><p>© 2026 Buyniverse Technologies. {{ store.t("Todos los derechos reservados.") }}</p></div></footer></div><div v-else class="relative flex h-screen overflow-hidden"><button v-if="mobileOpen" class="fixed inset-0 z-30 bg-slate-950/50 backdrop-blur-xs md:hidden" aria-label="Close navigation" @click="mobileOpen = false"></button><AppSidebar
        :menu="menu"
        :collapsed="collapsed"
        :mobile-open="mobileOpen"
        @close-mobile="mobileOpen = false"
        @toggle-collapse="collapsed = !collapsed"
      /><div class="relative z-10 flex min-w-0 flex-1 flex-col overflow-hidden"><header class="z-20 flex h-16 flex-shrink-0 items-center justify-between border-b border-slate-200/80 bg-white/80 px-4 backdrop-blur-md dark:border-slate-800/80 dark:bg-slate-900/80 sm:px-6 lg:px-8"><div class="flex items-center gap-3"><button class="text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 md:hidden" aria-label="Toggle navigation" @click="toggleNav"><i class="fa-solid fa-bars text-xl"></i></button><button class="grid h-9 w-9 place-items-center rounded-xl border border-slate-200/80 bg-slate-50 text-slate-500 md:hidden dark:border-slate-800 dark:bg-slate-800" aria-label="Quick access" @click="commandOpen = true"><i class="fa-solid fa-magnifying-glass text-xs"></i></button><button class="hidden items-center gap-2.5 rounded-xl border border-slate-200/90 dark:border-slate-700/80 bg-white dark:bg-slate-900/90 px-3.5 py-2 text-xs font-semibold text-slate-500 hover:border-brand hover:text-brand md:flex shadow-xs transition" @click="commandOpen = true"><i class="fa-solid fa-magnifying-glass text-xs"></i><span>Quick access</span><kbd class="rounded-md bg-slate-100 px-1.5 py-0.5 text-[9px] font-mono font-bold text-slate-500 dark:bg-slate-800 dark:text-slate-400">Ctrl K</kbd></button><span class="hidden items-center gap-2 rounded-xl border border-slate-200/90 bg-slate-100/70 px-3 py-1.5 text-[11px] font-700 text-slate-600 lg:flex dark:border-slate-700/80 dark:bg-slate-800/70 dark:text-slate-300" title="Active company workspace"><i class="fa-solid text-brand text-xs" :class="marketplaceMode === 'buyer' ? 'fa-cart-shopping' : marketplaceMode === 'supplier' ? 'fa-store' : 'fa-shield-halved'"></i>
              {{ activeModeLabel }}
            </span></div><div class="flex items-center gap-3 sm:gap-4"><span class="hidden items-center gap-1.5 text-[11px] font-medium lg:flex" :class="ui.saveState === 'error' ? 'text-rose-500' : 'text-slate-400'" :title="ui.lastSavedAt ? `Last local save ${ui.lastSavedAt}` : 'Local save status'"><i class="fa-solid text-xs" :class="ui.saveState === 'saving' ? 'fa-circle-notch animate-spin' : ui.saveState === 'error' ? 'fa-triangle-exclamation' : 'fa-cloud-arrow-up'"></i>
              {{ ui.saveState === "saving" ? "Saving…" : ui.saveState === "error" ? "Save failed" : "Saved locally" }}
            </span><RouterLink v-if="marketplaceMode === 'buyer'" to="/post-job/new" class="btn-brand hidden text-xs py-2 px-3.5 sm:inline-flex"><i class="fa-solid fa-plus text-xs mr-1.5"></i>{{ store.t("Post a Job") }}
            </RouterLink><RouterLink v-else-if="marketplaceMode === 'supplier'" to="/browse-services" class="btn-brand hidden text-xs py-2 px-3.5 sm:inline-flex"><i class="fa-solid fa-briefcase text-xs mr-1.5"></i>{{ store.t("Find Work") }}
              ...
              >
                {{ user.avatar }}
              </button><div v-if="accountOpen" class="absolute right-0 top-11 z-50 max-h-[calc(100vh-5rem)] w-76 overflow-y-auto rounded-2xl border border-slate-200/90 bg-white shadow-elevated dark:border-slate-700 dark:bg-slate-900"><div class="border-b border-slate-100 p-4 dark:border-slate-800"><p class="font-head font-bold text-sm">{{ user.name }}</p><p class="mt-0.5 truncate text-xs text-slate-500 dark:text-slate-400">{{ user.email }}</p></div><div class="py-1"><RouterLink :to="`/profile/${user.id}`" class="flex items-center px-4 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800 transition" @click="accountOpen = false"><i class="fa-regular fa-user mr-2.5 w-4 text-slate-400"></i>View profile
                  </RouterLink><RouterLink to="/profile/billing" class="flex items-center px-4 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800 transition" @click="accountOpen = false"><i class="fa-regular fa-credit-card mr-2.5 w-4 text-slate-400"></i>Billing & folios
                  </RouterLink><button class="flex w-full items-center px-4 py-2.5 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800 transition" @click="lockNow"><i class="fa-solid fa-lock mr-2.5 w-4 text-slate-400"></i>Lock workspace
                  </button></div><section v-if="marketplaceModeOptions.length > 1" class="border-t border-slate-100 p-3.5 dark:border-slate-800"><h2 class="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Company workspace</h2><div class="grid gap-1 rounded-xl bg-slate-100 p-1 dark:bg-slate-800/60" :class="marketplaceModeOptions.length > 2 ? 'grid-cols-3' : 'grid-cols-2'" role="group"><button
                    ...
                    ><i class="fa-solid text-xs" :class="option.icon"></i>{{ option.label }}
                      ...
                      >
                        {{ code.toUpperCase() }}
                      </button></div></div><div class="mt-3 flex items-center justify-between gap-3"><span class="text-xs font-semibold text-slate-700 dark:text-slate-300">Theme</span><button class="flex items-center gap-2 rounded-lg bg-slate-100 px-2.5 py-1.5 text-[11px] font-semibold text-slate-700 hover:text-brand dark:bg-slate-800/60 dark:text-slate-300 transition" @click="toggleTheme"><i class="fa-solid" :class="dark ? 'fa-moon' : 'fa-sun'"></i>
                      {{ dark ? "Dark mode" : "Light mode" }}
                    </button></div></section><div class="border-t border-slate-100 p-3.5 dark:border-slate-800"><label class="mb-1.5 block text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Demo account</label><select class="field py-2 text-xs" :value="store.state.currentUserId" @change="switchUser($event.target.value)"><option v-for="person in store.state.users" :key="person.id" :value="person.id">
                      {{ person.name }} · {{ person.type }}
                    </option></select></div></div></div></div></header><main class="relative z-10 flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8"><div class="mx-auto" :class="fullWidth ? 'max-w-none' : 'max-w-7xl'"><Breadcrumbs /><RouterView :key="route.path" /></div></main></div></div><div class="pointer-events-none fixed bottom-4 right-4 z-80 flex flex-col gap-2"><TransitionGroup name="toast"><div
        ...
        ><i class="fa-solid text-brand" :class="toast.icon || 'fa-circle-check'"></i><span>{{ toast.text }}</span></div></TransitionGroup></div></div></template>
<script>
